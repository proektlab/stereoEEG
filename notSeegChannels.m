function ibad = notSeegChannels(channel_labels,wm,varargin)
% function ibad = notSeegChannels(channel_labels,wm,bad_channels)
%   Returns indices of non-sEEG channels. It will look through
%   channel_labels and find all the channels that typically are not used in
%   our sEEG analyses (e.g. scalp EEG channels) and return indices for
%   these. If second input is set to 1, will also exclude channels with
%   white matter labels. If third input is given, it will also exclude
%   channels listed in bad_channels cell array.
%   
%   channel_labels (ch x [electrode name, anatomical label (optional)] cell array)
%   wm = 0 or 1 (include or exclude white matter channels?)
%   bad_channels = cell array of previously identified bad channel labels
%
%   DR 01/2023

ibad = [];
for ii = 1:length(channel_labels)
    elec = channel_labels{ii,1};
    if contains(elec,'C3') % exclude scalp EEG
        ibad = [ibad ii];
    elseif contains(elec,'C4')
        ibad = [ibad ii];
    elseif contains(elec,'Cz')
        ibad = [ibad ii];
    elseif contains(elec,'CZ')
        ibad = [ibad ii];
    elseif contains(elec,'Fp1')
        ibad = [ibad ii];
    elseif contains(elec,'Fp2')
        ibad = [ibad ii];
    elseif contains(elec,'F3')
        ibad = [ibad ii];
    elseif contains(elec,'F4')
        ibad = [ibad ii];
    elseif contains(elec,'F7')
        ibad = [ibad ii];
    elseif contains(elec,'F8')
        ibad = [ibad ii];
    elseif contains(elec,'Fz')
        ibad = [ibad ii];
    elseif contains(elec,'FZ')
        ibad = [ibad ii];
    elseif contains(elec,'P3')
        ibad = [ibad ii];
    elseif contains(elec,'P4')
        ibad = [ibad ii];
    elseif contains(elec,'Pz')
        ibad = [ibad ii];
    elseif contains(elec,'PZ')
        ibad = [ibad ii];
    elseif contains(elec,'O1')
        ibad = [ibad ii];
    elseif contains(elec,'O2')
        ibad = [ibad ii];
    elseif contains(elec,'T3')
        ibad = [ibad ii];
    elseif contains(elec,'T4')
        ibad = [ibad ii];
    elseif contains(elec,'T5')
        ibad = [ibad ii];
    elseif contains(elec,'T6')
        ibad = [ibad ii];
    elseif contains(elec,'LOC') % exclude EOG
        ibad = [ibad ii];
    elseif contains(elec,'ROC')
        ibad = [ibad ii];
    elseif contains(elec,'EMG') % exclude EMG
        ibad = [ibad ii];
    elseif contains(elec,'EKG') % exclude EKG
        ibad = [ibad ii];
    elseif contains(elec,'ECG')
        ibad = [ibad ii];
    elseif contains(elec,'SMELL') % exclude SMELL signal
        ibad = [ibad ii];
    elseif contains(elec,'RESP') % exclude RESP signal
        ibad = [ibad ii];
    elseif contains(elec,'DC') % exclude DC (analog input channels)
        ibad = [ibad ii];
    end
    if size(channel_labels,2)>1
        labl = channel_labels{ii,2};
        if isempty(labl) % exclude channels without label (usually outside of brain)
            ibad = [ibad ii];
        elseif contains(labl,'Unknown')
            ibad = [ibad ii];
        elseif wm && contains(labl,'White Matter') % exclude white matter-labeled channels
            ibad = [ibad ii];
        elseif wm && contains(labl,'White-Matter')
            ibad = [ibad ii];
        end
    end
end
if nargin==3
    bad_channels = varargin{1};
    bad = zeros(length(channel_labels),1);
    for ii = 1:length(bad_channels)
        bad = bad + contains(channel_labels(:,1),strtrim(bad_channels{ii}));
    end
    ibad = [ibad, find(bad)'];
end
ibad = sort(unique(ibad));